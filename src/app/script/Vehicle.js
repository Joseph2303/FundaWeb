...
const tabla = document.getElementById("vehicle-table");
...
function filtrar() {
    var inputBusqueda = document.getElementById('busquedaVehicle');
...
eliminarButton.addEventListener('click', async function (event) {
    ...
    if (checkboxes.length > 0) {
        ...
        if (confirm(`¿Está seguro de que desea eliminar el cliente con cédula ${placa}?`)) {
            try {
                 destroy(placa);
            } catch (error) {
        ...
        alert('Por favor, seleccione un vehiculo para eliminar.');
    }
});
...
actualizarButton.addEventListener('click', function (event) {
    ...
    if (checkboxes.length > 0) {
        ...
        const vehicleDataString = filaSeleccionada.getAttribute('data-vehicle');
        const placa = filaSeleccionada.getAttribute('data-plca');
